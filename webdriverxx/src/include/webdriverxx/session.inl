...
inline
const Session& Session::SetPermissions(const std::string& id, const std::string& name, const std::string& value) const {
    if (id.empty()) {
        throw WebDriverException("Chromium app id must not be null or the empty string");
    }
...
inline
const Session& Session::SetNetworkConditions(const std::string& id, bool offline, int latency, int download_throughput,
											 int upload_throughput) const {
	if (id.empty()) {
		throw WebDriverException("Chromium app id must not be null or the empty string");
	}
