...
public:
	...
	const Session& SetPermissions(const std::string& id, const std::string& name, const std::string& value) const;
	picojson::value GetNetworkConditions() const;
	void DeleteNetworkConditions() const;
	const Session& SetNetworkConditions(const std::string& id, bool offline, int latency, int download_throughput,
