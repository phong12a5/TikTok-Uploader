...
public class Xposed implements IXposedHookLoadPackage {
    ...
    public void handleLoadPackage(XC_LoadPackage.LoadPackageParam lpparam) {
        ...
        if (lpparam.packageName.equals(Constants.REG_PACKAGE)) {
            ...
            try {
                ...
                generateTelephonyInfo();

                XposedBridge.log("buildPropObject: " +  buildPropObject);
                XposedBridge.log("otherObject: " +  otherObject);
                XposedBridge.log("telephonyObject: " +  telephonyObject);
    ...
    private void hookAcccountManager() {
        XposedBridge.log("hookAcccountManager");
    ...
    private void importDevice() {
        XposedBridge.log("importDevice");
        try {
            String jsonContent = readFile(Constants.DEVICE_INFO_FILE);
            JSONObject devices_info = new JSONObject(jsonContent);
            otherObject = devices_info.getJSONObject("other");
            buildPropObject = devices_info.getJSONObject("buildProp");
        } catch (Exception e) {}
    }

    private void generateRandomInfo() {
        XposedBridge.log("generateRandomInfo");
        try {
    ...
    private void generateTelephonyInfo() {
        try {
            telephonyObject = new JSONObject();
